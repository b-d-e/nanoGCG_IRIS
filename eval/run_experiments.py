...
# Import your nanogcg module (adjust path as needed)
sys.path.append('..')  # Adjust this path to where nanogcg is located
import nanogcg
from nanogcg import GCGConfig

# Configuration constants
WANDB_ENTITY = "reasoning_attacks"
WANDB_PROJECT = "gcg-iris-eval-fix"
CAUTION_VECTOR = "../gcg/cautious_dir.pt"
...
INPUT_CSV = "../gcg/orthogonalized_outputs_cot150_2048.csv"
...
def worker_init(gpu_id: int):
    ...
    try:
        ...
        print(f"Worker on GPU {gpu_id}: Model loaded. Memory - Allocated: {allocated:.2f}GB, Reserved: {reserved:.2f}GB")
...
def load_input_data(csv_path: str) -> pd.DataFrame:
...
def preprocess_target_strings(df: pd.DataFrame, target_tokens: int, use_override: bool = False) -> pd.DataFrame:
...
def get_output_csv_path(results_dir: str, beta: float, target_tokens: int, refusal_tokens: int, use_override: bool = False) -> str:
    ...
    if use_override:
        return os.path.join(results_dir, f"gcg_results_beta_{beta_str}_override_target_refusal_{refusal_tokens}_{timestamp}.csv")
    else:
        return os.path.join(results_dir, f"gcg_results_beta_{beta_str}_target_{target_tokens}_refusal_{refusal_tokens}_{timestamp}.csv")
...
def setup_wandb_config(beta: float, prompt_idx: int, gpu_id: int, target_tokens: int, refusal_tokens: int, num_steps: int, forbidden_prompt: str, use_override: bool = False) -> Dict:
...
def monitor_memory(stage: str):
    ...
    if torch.cuda.is_available():
        allocated = torch.cuda.memory_allocated(0) / 1e9
        reserved = torch.cuda.memory_reserved(0) / 1e9
        print(f"GPU {_worker_gpu_id} [{stage}]: Allocated: {allocated:.2f}GB, Reserved: {reserved:.2f}GB")
...
def run_single_experiment(args: Tuple) -> Dict:
    ...
    (prompt_idx, forbidden_prompt, target_string, beta, target_tokens, refusal_tokens, assigned_gpu_id, verbose, num_steps, refusal_layer_idx, output_csv, use_override) = args
    ...
    try:
        ...
        # Set up wandb config
        wandb_config = setup_wandb_config(beta, prompt_idx, _worker_gpu_id, target_tokens, refusal_tokens, num_steps, forbidden_prompt, use_override)
        ...
        print(f"GPU {_worker_gpu_id}: Starting experiment {prompt_idx} with beta={beta}")
        if use_override:
        ...
        experiment_result = {
            ...
            'loss_improvement': float(all_losses[0] - all_losses[-1]) if len(all_losses) > 1 else 0.0,
            ...
            'optimization_strings_sample': '|'.join(all_strings[:5]) if all_strings else ''
        }
        ...
        print(f"GPU {_worker_gpu_id}: Completed experiment {prompt_idx} in {runtime:.1f}s")
        return experiment_result
        
    except torch.cuda.OutOfMemoryError as e:
        error_msg = f"CUDA OOM on GPU {_worker_gpu_id} for experiment {prompt_idx}: {str(e)}"
        print(error_msg)
        ...
        error_msg = f"Error in experiment {prompt_idx}: {str(e)}\n{traceback.format_exc()}"
...
def prepare_experiments(df: pd.DataFrame, beta: float, target_tokens: int, refusal_tokens: int, existing_results: pd.DataFrame, verbose: bool, num_gpus: int, num_steps: int, refusal_layer_idx: int, output_csv: str, use_override: bool) -> List[Tuple]:
    """Prepare list of experiments to run, excluding already completed ones."""
    
    experiments = []
    completed_indices = set(existing_results['prompt_idx'].tolist()) if not existing_results.empty else set()
...
def main():
    ...
    input_df = preprocess_target_strings(input_df, args.num_target_tokens, args.target_override)
    
    output_csv = get_output_csv_path(args.results_dir, args.beta, args.num_target_tokens, args.num_refusal_tokens, args.target_override)
    existing_results = load_existing_results(output_csv)
    
    experiments = prepare_experiments(input_df, args.beta, args.num_target_tokens, args.num_refusal_tokens, existing_results, args.verbose, args.num_gpus, args.num_steps, args.refusal_layer_idx, output_csv, args.target_override)
    ...
    target_info = f"override target ('{OVERRIDE_TARGET_STRING}')" if args.target_override else f"{args.num_target_tokens} target tokens"
    print(f"Planning to run {len(experiments)} experiments with beta={args.beta}")
    print(f"Using {args.num_gpus} GPUs, {target_info}, {args.num_refusal_tokens} refusal tokens")
    ...
    with mp.Pool(processes=args.num_gpus) as pool:
        ...
        gpu_experiment_pairs = [(gpu_id, experiments_per_gpu[gpu_id]) 
                               for gpu_id in range(args.num_gpus) 
    ...
    try:
        ...
        print(f"Final results file sorted and saved with {len(final_df)} total experiments")
    except Exception as e:
