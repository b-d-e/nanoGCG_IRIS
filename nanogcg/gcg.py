...
class GCG:
    ...
    def _setup_refusal_direction(self):
        ...
        if config.refusal_vector is not None:
            self.refusal_vector = config.refusal_vector.to(self.model.device, self.model.dtype)
            ...
            try:
                ...
                self.refusal_vector = torch.load(config.refusal_vector_path, map_location=self.model.device)
                ...
                try:
                    ...
                    self.refusal_vector = torch.tensor(refusal_array, device=self.model.device)
        ...
        self.think_start_id = self.tokenizer.encode(config.think_start_token, add_special_tokens=False)[0]
        self.think_end_id = self.tokenizer.encode(config.think_end_token, add_special_tokens=False)[0]
    ...
    def _find_cot_token_positions(self, input_ids: Tensor) -> Tuple[Optional[int], Optional[int]]:
        """
        ...
        think_start_positions = (input_ids == self.think_start_id).nonzero(as_tuple=True)[0]
    ...
    def compute_loss_with_refusal_direction(self, shift_logits, shift_labels, input_embeds):
        """
        Compute combined loss: original cross-entropy + refusal direction term.
        """
